...
def skill_trust(skill):
    """
    How 'earned' a skill looks, based on endorsements and how long they've
    held it - not just whether the word is on the list. A skill someone
    picked up 5 months ago with 0-1 endorsements (classic 'taking an online
    course, side project' buzzword) counts for much less than a skill with
    real endorsements and years of use, even if both match the same keyword.
    """
    try:
    ...
    trust = (endorsements / 10) * 0.5 + (duration / 24) * 0.5
    trust = min(trust, 1.0)

    # floor so a genuine but newer skill still counts for something
    return max(trust, 0.2)
...
def score_career(candidate):
    ...
    if any(x in title for x in GOOD_TITLES):
        current = 1
    elif any(x in title for x in PARTIAL_TITLES):
        current = 0.6
    else:
        current = 0.3

    # Also check past roles - a candidate with one shiny current title but
    # no AI/ML history at all is a weaker signal than someone whose whole
    # career has been building toward this.
    history = candidate.get("career_history", [])
    past_titles = [str(h.get("title","")).lower() for h in history]
    has_past_ai_title = any(
        any(x in t for x in GOOD_TITLES) for t in past_titles
    )

    if current == 1 and not has_past_ai_title and not past_titles:
        # no career history at all to corroborate - keep as is, can't penalize
        return current
    if current < 1 and has_past_ai_title:
        # past AI experience but a non-AI current title - give partial credit back
